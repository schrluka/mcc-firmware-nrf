...
void l1_init()
{
    ...
    float kp = 0.9 * PWM_PERIOD / 3300;    // rescale from voltage to duty cycle by /3300mV and multiply with PWM period to get compare value

    set_pi_gains(&current_ctrl, kp, 500, F_FAST_CTRL);
    //printf("current ctrl: kI: %d   kP: %d   denom: %d \n", (int)current_ctrl.k_i, (int)current_ctrl.k_p, (int)current_ctrl.denom);
...
//	int32_t iErr;
    ...
    if (u_ctrl_cnt != 0) {
        // Note: don't execute current controller when PWM is off to preserve integrator state
